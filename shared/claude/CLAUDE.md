...
Workflow: pipe the bundle into `~/.claude/bin/codex-second-opinion.sh` via a quoted heredoc (so `$vars` and backticks don't expand). The script handles the codex invocation and writes verdict JSON to `/tmp/codex-options.json`:

```bash
~/.claude/bin/codex-second-opinion.sh <<'EOF'
## Question / problem
...
2. **Run `~/.claude/bin/codex-precommit.sh`** (no args, no stdin needed). It bundles the active session transcript + the staged diff, calls codex with the schema, and writes verdict JSON to `/tmp/codex-review.json`. Sizing, transcript discovery, codex flags, and the prompt body all live in the script. Two policy reminders the script can't enforce:
   - Codex auto-loads `AGENTS.md` from cwd; do not cat it into the prompt. If a project section deserves focus (e.g. realtime audio rules, append-only migrations), name it in the prompt by section number; cheaper than re-passing the file.
   - If the transcript is missing (fresh session, restored cwd, committing from a non-session shell), codex falls back to diff-only review. Note it in your reply.
...
## Plugins
Active globally: claude-mem, context7, rust-analyzer-lsp, typescript-lsp. Use their skills/commands when they match; don't reinvent what they provide.
